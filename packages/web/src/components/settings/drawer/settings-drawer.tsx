...
export function SettingsDrawer({ sx, defaultSettings }: SettingsDrawerProps) {
  ...
  const renderMode = () => {
    const getNextMode = (currentMode: 'light' | 'dark' | 'system') => {
      switch (currentMode) {
        case 'light':
          return 'dark';
        case 'dark':
    ...
    const getModeIcon = (currentMode: 'light' | 'dark' | 'system') => {
      switch (currentMode) {
        case 'light':
          return settingIcons.sun;
        case 'dark':
          return settingIcons.moon;
        case 'system':
          return settingIcons.system;
        default:
          return settingIcons.sun;
      }
    };
    ...
    return (
      <BaseOption
        ...
        icon={<SvgIcon>{getModeIcon(settings.state.mode)}</SvgIcon>}
        action={
          settings.state.mode === 'system' ? (
            <Label
              sx={{
                height: 20,
                cursor: 'inherit',
        ...
        onChangeOption={() => {
          const nextMode = getNextMode(settings.state.mode);
          setMode(nextMode);
